...
void desenhaFim(int campo[60][80])
{
    for (int i = 0; i < 60; i++)
    {
        for (int j = 0; j < 80; j++)
        {
            ...
            int block_col = j;
            int block_line = i;
            set_background_block(block_col, block_line, B, G, R); // encontro da coluna e linha
        }
    }
}

void desenhaGameOver(int campo[60][60])
{
    for (int i = 0; i < 60; i++)
    {
        for (int j = 0; j < 60; j++)
...
void desenharSprite()
{
    for (int i = 0; i < 20; i++)
    {
        ...
        for (int j = 0; j < 20; j++)
        {
            set_pixelSprite(0 + i + (j * 20), pacman[j][i]);   // setando o pacman no primeiro sprite
            set_pixelSprite(400 + i + j * 20, fantasma[j][i]); // setando o fantasma no segundo sprite
            set_pixelSprite(800 + i + j * 20, pacmanFechada[j][i]); // setando o pacman fechado no segundo sprite
        }
    }
}
...
int main()
{
    ...
    {   
        ...
        case (4):
            ...
            campoAtivo[18][35] = 0b111000000;
            //set_background_block(1,55,0,0,0);
            for (int i = 1; i < 4; i++){
                for(int j = 55; j < 59; j ++){
                    campoAtivo[j][i] = 0b000000000;
                    //campoAtivo[j][i + 31] = 0b000000000; 
                }
            }
            for (int i = 33; i <= 37; i++){
                for(int j = 55; j < 59; j ++){
